...
class ExistingUsernameValidationTest {

    @InjectMocks
    private ExistingUsernameValidation usernameValidation;
    ...
    @Test
    void execute_ChecksIfUsernameAlreadyExists() {
    ...
    @Test
    void execute_ThrowsBadRequest_WhenUsernameExists() {
        BDDMockito.when(userRepositoryMock.existsByUsername(ArgumentMatchers.anyString()))
                .thenReturn(true);

        assertThatExceptionOfType(BadRequestException.class)
                .isThrownBy(() -> usernameValidation.execute(createDoctorRequest()))
                .withMessageContaining(MESSAGE);

        assertThatExceptionOfType(BadRequestException.class)
                .isThrownBy(() -> usernameValidation.execute(createUserRequest()))
                .withMessageContaining(MESSAGE);

        assertThatExceptionOfType(BadRequestException.class)
                .isThrownBy(() -> usernameValidation.execute(createAdminRequest()))
                .withMessageContaining(MESSAGE);
    }
